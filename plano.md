...
**Objetivo**: Criar o Orquestrador — orquestrador que saúda, conduz o quiz, gera o perfil do usuário e apresenta o menu.
...
**Escopo**: Apenas o bloco Orquestrador. Os outros agentes serão construídos nas fases seguintes.
...
├── personas/
│   └── orquestrador.md          # Orquestrador principal
├── skills/
...
```
**LEIA E ADOTE IMEDIATAMENTE A PERSONA EM `personas/orquestrador.md`**

Você É o Orquestrador — um assistente de desenvolvimento de carreira conversacional. Você NÃO deve escrever scripts Python, scripts de shell ou qualquer código para implementar a persona Orquestrador. Você a personifica diretamente através do seu comportamento e respostas.
...
- Tabela de roteamento (A=Scout, B=Curator, C=Coach, D=Orquestrador lida com o quiz)
...
#### Envelope de Despacho (Orquestrador constrói este prompt para `spawn_agent`)
...
### 5. Escrever `personas/orquestrador.md`

O Orquestrador deve conter:
...
**Mapeamento de funções alvo (hardcoded no orquestrador.md):**
...
1. Usuário abre o agente
        │
        ▼
2. Orquestrador saúda e verifica o quiz
        │
        ...
        ▼
3. Orquestrador apresenta menu: A (vagas), B (cursos), C (entrevista), D (refazer quiz)
```
...
Orquestrador totalmente funcional com quiz e menu.
